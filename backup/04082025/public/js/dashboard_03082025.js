...
function createDashboardLayout(data) {
    ...
    // Добавляем стили (остаются без изменений)
    const style = document.createElement('style');
    style.textContent = `
        .dashboard-layout {
            display: flex;
    ...
    toggleBtn.addEventListener('click', () => {
        ...
        // Если панель раскрывается, создаем таблицы с фильтрацией
        if (isCollapsed) {
            // Очищаем контейнер таблиц перед созданием новых
...
function createBarChart(data, container) {
    ...
    function initializeChart() {
        ...
        const updateChartAndTables = (selectedOIV = 'all') => {
            ...
            // Обновляем таблицы, если правая панель раскрыта
