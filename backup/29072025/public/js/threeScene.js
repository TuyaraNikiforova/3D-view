...
function applyFiltersToScene(filters = {}) {
    ...
    // 2. Определяем, какие OIV должны быть видны на основе всех фильтров
    const visibleOIVs = new Set();
    
    // Фильтрация по OIV (если выбраны конкретные OIV)
    if (filters.oiv && filters.oiv.length > 0) {
        filters.oiv.forEach(oivId => {
            visibleOIVs.add(oivId);
        });
    } else {
        // Если OIV не выбраны, рассматриваем все OIV
        data.oiv.forEach(oiv => {
            visibleOIVs.add(oiv.id);
        });
    }
    ...
    if (filters.complexes && filters.complexes.length > 0) {
        data.oiv.forEach(oiv => {
            if (!filters.complexes.includes(oiv.complex)) {
                visibleOIVs.delete(oiv.id);
            }
        });
    }
    ...
    if (filters.themes && filters.themes.length > 0) {
        ...
        data.edges.forEach(edge => {
            if (filters.themes.includes(edge.theme)) {
                themeOIVs.add(edge.source);
                themeOIVs.add(edge.target);
            }
        });
        
        // Оставляем только OIV, которые есть в обоих наборах
        visibleOIVs.forEach(oivId => {
            if (!themeOIVs.has(oivId)) {
                visibleOIVs.delete(oivId);
            }
        });
    }

    // 3. Показываем только выбранные OIV
    Array.from(visibleOIVs).forEach(oivId => {
        const node = nodeMeshes[oivId];
        if (node) {
            node.visible = true;
            
            // Показываем комплекс этого OIV
            const complexId = node.userData.complex;
            const complex = complexSpheres[complexId];
            if (complex) {
                complex.visible = true;
                complex.material.opacity = selectedComplexIds.includes(complexId) ? 0.8 : 0.6;
                complex.material.needsUpdate = true;
            }
        }
    });

    // 4. Показываем только связи между видимыми OIV и соответствующие темам
    data.edges.forEach(edge => {
        const lineKey = `${edge.source}-${edge.target}`;
        const line = edgeLines[lineKey];
        if (!line) return;
        
        const shouldShow = visibleOIVs.has(edge.source) && 
                         visibleOIVs.has(edge.target) &&
                         (!filters.themes || filters.themes.length === 0 || 
                          filters.themes.includes(edge.theme));
        
        line.visible = shouldShow;
        if (shouldShow) {
            line.material.opacity = filters.themes && filters.themes.includes(edge.theme) ? 1.0 : 0.8;
            line.material.needsUpdate = true;
        }
    });

    // 5. Центрируем камеру на видимых элементах
    ...
    // Добавляем видимые OIV
    Array.from(visibleOIVs).forEach(oivId => {
        const node = nodeMeshes[oivId];
        if (node && node.visible) visibleObjects.push(node);
    });
    ...
    data.edges.forEach(edge => {
        const lineKey = `${edge.source}-${edge.target}`;
        const line = edgeLines[lineKey];
        if (line && line.visible) visibleObjects.push(line);
    });
