...
function createAdvancedFilterModal() {
    ...
    modal.style.display = 'none'; // Добавляем начальное скрытое состояние
    modal.innerHTML = `
        <div class="advanced-filter-modal-content">
            <div class="advanced-filter-modal-header">
                <div class="advanced-filter-modal-title">
                    <span>Расширенный фильтр</span>
                </div>
                <button class="advanced-filter-modal-close">&times;</button>
            </div>
            <div class="advanced-filter-modal-search-container">
                <input type="text" id="advanced-modal-search-input" placeholder="Поиск..." class="advanced-filter-modal-search">
            </div>
            <div class="advanced-filter-modal-body" id="advanced-modal-filter-container"></div>
            <div class="advanced-filter-modal-footer">
                <button class="advanced-filter-modal-btn reset">Сбросить все</button>
                <button class="advanced-filter-modal-btn apply">Применить</button>
            </div>
        </div>
    `;
    ...
    style.textContent = `
        ...
        .advanced-filter-modal-search-container {
            padding: 10px 20px;
            background-color: #333;
        }
        
        .advanced-filter-modal-search {
            width: 100%;
            padding: 8px 15px;
            border-radius: 20px;
            border: none;
        ...
        .advanced-filter-modal-btn.reset {
            background-color: #555;
            color: #fff;
        }
        
        .advanced-filter-modal-btn.apply {
            background-color: #4a6da7;
            color: #fff;
        }
        
        .advanced-filter-item.hidden {
            display: none;
        }
    `;
    document.head.appendChild(style);
    ...
    // Обработчик поиска
    const searchInput = modal.querySelector('#advanced-modal-search-input');
    searchInput.addEventListener('input', function() {
        const searchTerm = this.value.toLowerCase();
        const filterItems = modal.querySelectorAll('.advanced-filter-item');
    ...
    const closeModal = () => {
        modal.style.display = 'none';
        searchInput.value = '';
        modal.querySelectorAll('.advanced-filter-item').forEach(item => item.classList.remove('hidden'));
    };
    
    modal.querySelector('.advanced-filter-modal-close').addEventListener('click', closeModal);
    
    // Обработчик для кнопки "Сбросить все"
    modal.querySelector('.advanced-filter-modal-btn.reset').addEventListener('click', () => {
    ...
    // Обработчик для кнопки "Применить"
	modal.querySelector('.advanced-filter-modal-btn.apply').addEventListener('click', () => {
		// Собираем все выбранные фильтры
		const filterTypes = ['complex', 'oiv', 'theme', 'strategy', 'program', 'project'];
		const selectedFilters = {};
		
		filterTypes.forEach(type => {
			const checkboxes = modal.querySelectorAll(`input[data-type="${type}"][type="checkbox"]:checked`);
			selectedFilters[`${type}s`] = Array.from(checkboxes).map(cb => cb.value);
		});
		
		// Сбрасываем все текущие фильтры в основном интерфейсе
		document.querySelectorAll('.filter-item input[type="checkbox"]').forEach(checkbox => {
			checkbox.checked = false;
		});
		
		// Применяем выбранные фильтры к основному интерфейсу
		filterTypes.forEach(type => {
			const containerId = `${type}-filters`;
			const checkboxes = document.querySelectorAll(`#${containerId} input[type="checkbox"]`);
			
			checkboxes.forEach(checkbox => {
				if (selectedFilters[`${type}s`].includes(checkbox.value)) {
					checkbox.checked = true;
					// Триггерим событие change для каждого выбранного чекбокса
			...
			// Обновляем счетчики
			updateFilterCount(containerId, selectedFilters[`${type}s`].length);
			
			// Вызываем соответствующие функции выделения
			if (type === 'complex' && window.updateSelectedComplexes) {
				window.updateSelectedComplexes(selectedFilters.complexes);
			} else if (type === 'oiv' && window.selectOIV) {
				window.selectOIV(selectedFilters.oiv);
			} else if (type === 'theme' && window.selectTheme) {
				window.selectTheme(selectedFilters.themes);
			} else if (type === 'strategy' && window.selectStrategy) {
				window.selectStrategy(selectedFilters.strategies);
			} else if (type === 'program' && window.selectProgram) {
				window.selectProgram(selectedFilters.programs);
			} else if (type === 'project' && window.selectProject) {
				window.selectProject(selectedFilters.projects);
			}
		});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initFilters(data) {
        ...
        // Создаем кнопку "Сбросить все фильтры"
        ...
        // Создаем кнопку "Расширенный фильтр"
        const advancedFilterBtn = document.createElement('button');
        advancedFilterBtn.id = 'advanced-filter-btn';
        advancedFilterBtn.className = 'filter-reset-btn';
        advancedFilterBtn.textContent = 'Расширенный фильтр';
        advancedFilterBtn.title = 'Открыть расширенный фильтр';
        
		advancedFilterBtn.addEventListener('click', function(e) {
			e.preventDefault();
			e.stopPropagation();
			const modal = document.querySelector('.advanced-filter-modal') || createAdvancedFilterModal();
			const filterContainer = modal.querySelector('#advanced-modal-filter-container');
			filterContainer.innerHTML = '';
			
			// Добавляем все группы фильтров в модальное окно
			const filterTypes = [
				{ id: 'complex-filters', name: 'Комплексы' },
				{ id: 'oiv-filters', name: 'ОИВ' },
				{ id: 'theme-filters', name: 'Темы' },
				{ id: 'strategy-filters', name: 'Стратегии' },
				{ id: 'program-filters', name: 'Госпрограммы' },
				{ id: 'project-filters', name: 'Проекты' }
			];
			...
			filterTypes.forEach(type => {
				...
				groupDiv.className = 'advanced-filter-group';
				
				const title = document.createElement('div');
				title.className = 'advanced-filter-group-title';
				title.textContent = type.name;
				groupDiv.appendChild(title);
				
				const filters = container.querySelectorAll('.filter-item');
				filters.forEach(filter => {
					const checkbox = filter.querySelector('input[type="checkbox"]');
					const label = filter.querySelector('label');
					...
					if (checkbox && label) {
						...
						itemDiv.className = 'advanced-filter-item';
						
						const newCheckbox = document.createElement('input');
						newCheckbox.type = 'checkbox';
						newCheckbox.id = `advanced-${checkbox.id}`;
						newCheckbox.value = checkbox.value;
						newCheckbox.dataset.type = checkbox.dataset.type;
						newCheckbox.checked = checkbox.checked;
        ...
        if (filterForm) {
            ...
            buttonsContainer.appendChild(advancedFilterBtn);
            filterForm.insertBefore(buttonsContainer, filterForm.firstChild);
            ...
            style.textContent = `
                ...
                #reset-all-filters, #advanced-filter-btn {
                    padding: 8px 16px;
                    background-color: #4a6da7;
                    color: white;
                    border: none;
                ...
                #reset-all-filters:hover:not(.disabled), #advanced-filter-btn:hover {
