			...
			visibleColumns.forEach(column => {
				switch(column) {
					...
					case 'Стратегия':
						...
						row[column] = allStrategies
							.map(strategy => strategy.name || '')
							.filter(Boolean)
							.join(', ');
						break;
					case 'Гос.программы':
						// Получаем все программы для sourceOIV из programs.json
						const allPrograms = window.programsData || [];
						row[column] = allPrograms
						...
						break;
					case 'Подпрограммы':
						// Получаем все подпрограммы для sourceOIV из programs.json
						const allSubprograms = window.programsData || [];
						row[column] = allSubprograms
	...
	function applyAllFilters(edges, data) {
		...
		if (window.currentSearchTerm) {
			...
			filteredEdges = filteredEdges.filter(edge => {
				...
				// Получаем все стратегии (из data.strategies и additionalStrategies)
				const edgeStrategies = window.additionalStrategies?.filter(s => s.oiv_id === edge.source) || [];
				const allStrategies = [
					...(sourceOIV.strategies || []).map(strategyId => {
						let strategy = data.strategies?.find(s => s.id === strategyId) || 
				...
				return (
					...
					allStrategies.some(strategy => strategy.name.toLowerCase().includes(searchTerm)) ||
		...
		Object.keys(window.currentColumnFilters).forEach(columnName => {
			...
			filteredEdges = filteredEdges.filter(edge => {
				...
				switch(columnName) {
					...
					case 'Стратегия':
						return allStrategies.some(strategy => 
							strategy.name.toLowerCase().includes(filterValue)
						);
					case 'Гос.программы':
