...
function calculateStatistics(filters) {
    ...
    if (filters?.programs?.length > 0) {
        ...
        const uniquePrograms = new Set(filters.programs);
        ...
        oivWithPrograms.forEach(oiv => {
            ...
            if (oiv.programs) {
                oiv.programs.forEach(p => uniquePrograms.add(p));
            }
        });
        ...
        stats.programs = uniquePrograms.size;
    ...
    if (filters?.edges?.length > 0) {
        ...
        selectedEdges.forEach(edge => {
            ...
            if (sourceOIV) {
                ...
                if (sourceOIV.programs) {
                    sourceOIV.programs.forEach(p => uniquePrograms.add(p));
                }
            }
            ...
            if (targetOIV) {
                ...
                if (targetOIV.programs) {
                    targetOIV.programs.forEach(p => uniquePrograms.add(p));
                }
            }
        });
    ...
    if ((filters?.oiv?.length > 0) || (filters?.oivIds?.length > 0)) {
        ...
        // Находим все уникальные комплексы, стратегии и программы
        const uniqueComplexes = new Set();
