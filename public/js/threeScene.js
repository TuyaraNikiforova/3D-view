...
window.applyTableViewFilters = function(filters) {
    ...
    window.pendingTableViewFilters = null;
};
...
window.switchTo3DView = function() {
    ...
    const threeDViewTab = document.querySelector('[data-tab="3d-view"]');
    if (threeDViewTab) {
        threeDViewTab.click();
    }
    
    // Применяем фильтры после загрузки 3D сцены
    setTimeout(() => {
        if (window.applyCascadeFilter) {
            window.applyCascadeFilter(currentFilters);
        }
    }, 500);
};
...
function addTableViewSwitchButton() {
    ...
    switchButton.id = 'switch-to-table';
    switchButton.innerHTML = '📊 Перейти к таблице';
    ...
    switchButton.style.background = 'rgba(74, 109, 167, 0.8)';
